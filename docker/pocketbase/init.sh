    ...
    migrate_field() {
        ...
        local col_data=$(curl -sf "$PB_URL/api/collections" \
            -H "Authorization: Bearer $TOKEN" \
            | jq ".items[] | select(.name == \"$collection\")")
        local col_id=$(echo "$col_data" | jq -r '.id // empty')

        if [ -z "$col_id" ]; then
            echo "  Collection '$collection' not found, skipping migration."
            return
        fi

        local exists=$(echo "$col_data" | jq ".fields[] | select(.name == \"$field_name\") | .name")
        if [ -n "$exists" ]; then
            echo "  Field '$field_name' already exists on '$collection', skipping."
            return
        fi

        echo "  Adding field '$field_name' to '$collection'..."
        local new_field="{\"name\":\"$field_name\",\"type\":\"$field_type\"}"
        local updated_fields=$(echo "$col_data" | jq ".fields + [$new_field]")
        local result=$(curl -sf -X PATCH "$PB_URL/api/collections/$col_id" \
            -H "Authorization: Bearer $TOKEN" \
            -H "Content-Type: application/json" \
            -d "{\"fields\":$updated_fields}" 2>&1)
        local updated_id=$(echo "$result" | jq -r '.id // empty')
        if [ -n "$updated_id" ]; then
            echo "    $field_name: added"
        else
            echo "    $field_name: FAILED ($result)"
        fi
    }

    migrate_field "clients" "settlementType" "text"
    migrate_field "tasks" "priority" "text"
